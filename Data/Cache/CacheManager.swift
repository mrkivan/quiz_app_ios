...
final class CacheManager {
    ...
    func getDashboard() -> DashboardDTO? {
        guard let data = userDefaults.data(forKey: Keys.dashboardData) else { return nil }
    ...
    func getQuiz(key: String) -> QuizListDTO? {
        guard let data = userDefaults.data(forKey: quizKey(key)) else { return nil }
    ...
    func clear() {
        ...
        for key in dictionary.keys where key.hasPrefix("quiz_") || key == Keys.dashboardData {
