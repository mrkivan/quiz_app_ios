...
class QuizViewModel: BaseViewModel<QuizData> {
    ...
    // MARK: - Init
    init(
        getQuizDataUseCase: GetQuizDataBySetAndTopicUseCase = Resolver.resolve(),
    ...
    private func fetchQuiz(quizScreenData: QuizScreenData) {
        ...
        setLoading()
        getQuizDataUseCase.execute(fileName: quizScreenData.quizSection?.fileName ?? "")
            .sink { [weak self] resource in
                ...
                case .success(let quizItems):
                    guard !quizItems.isEmpty else {
                        self?.setError(message: BaseViewModel<QuizData>.ERROR_DATA_NOT_FOUND)
                        return
                    }
                    ...
                    if let first = self?.cacheQuizList.first {
                        ...
                        self?.quizState = QuizState(
                            isLastItem: self?.isLastItem() ?? false, currentQuestionNumber: 1,
    ...
    private func submitAnswer() {
        ...
        let isCorrect = Set(currentQuiz.correctAnswer.answerId) == Set(quizState.selectedAnswers)
    ...
    private func moveToNextQuestion(isSkipped: Bool = false) {
        ...
        if nextIndex < cacheQuizList.count {
            ...
            quizState = QuizState(
                isLastItem: isLastItem(), currentQuestionNumber: currentQuizPosition + 1,
    ...
    private func navigateToResultScreen() {
        ...
        let resultData = ResultData(
            ...
            totalQuestions: cacheQuizList.count,
            resultPercentage: getResultPercentage(correctAnswers: correctAnswers)
        )
        
        saveResultDataUseCase.execute(key: data.quizSection?.fileName ?? "", result: resultData)
            .sink { [weak self] _ in
                self?.navigationEvents.send(.navigateToResult(key: self?.getResultKey() ?? ""))
            }
            .store(in: &cancellables)
    }
    ...
    private func getResultPercentage(correctAnswers: Int) -> Int {
        ...
        return totalQuestions > 0 ? Int((Double(correctAnswers) / Double(totalQuestions)) * 100) : 0
    }
    ...
    private func saveResult(isSkipped: Bool = false) {
        ...
        let resultItem = ResultData.Item(
            ...
            result: Set(currentQuiz.correctAnswer.answerId) == Set(quizState.selectedAnswers),
